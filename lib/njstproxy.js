        ...
        let origReq = requestor.request(reqOptions, (origResp) => {
            ...
            headers.forEach((val, key) => {
                ...
                try {
                    ...
                    let escapedVal = val.replace(/[^a-zA-Z0-9,.;=&@\-+\/()\\[\] ]+/g, (c) => { return escape(c); });
                    try {
            ...
            .once('error', (e) => {
                ...
                origResp.removeAllListeners('data');
                origResp.removeAllListeners('end');
            })
            .once('end', () => {
                debuglog('Original response [%d] processing finished.', reqSerialNo);
                this.emit('responseFinish', origResp);
                origResp.removeAllListeners('data');
                origResp.removeAllListeners('error');
                res.end();
            });
        })
        .once('error', (e) => {
